    ...
    def __init__(self, angle, spec, bounds_error=False):
        if np.all(spec > 0):
            self.interp_func = interp1d(
                ...
                fill_value=np.log(spec[:,0])#-np.inf,
            )
            ...
            self.interp_func = interp1d(
                np.log(angle), spec, bounds_error=bounds_error, fill_value=spec[:,0]#0.0
            )
...
def execute(block, config):
    ...
    if config["model_type"] == "pca":
        ...
        pars1 = pars_cor#np.hstack([pars_cor, pars_c1])
        pars2 = pars_cor#np.hstack([pars_cor, pars_c2])
        rr = config["r"]
    ...
    for i in range(n_a):
        for j in range(i, n_b):
            ...
            # RT: option: save delta xip
            block["delta_xi_plus", bn] = dxip[ib]
